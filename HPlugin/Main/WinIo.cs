...
namespace IPlugin.Main
{
    ...
    public class WinIo  
    {
        ...
        /// key down  
        public static void MykeyDown(VirtualKeyCode vKeyCoad)  
        {  
        ...
        /// Key up  
        public static void MykeyUp(VirtualKeyCode vKeyCoad)  
        {  
        ...
        {
            KBCWait4IBE();                               //'等待缓冲区为空
            SetPortVal(KBC_KEY_CMD, (IntPtr)211, 1);             //'发送鼠标写入命令
            KBCWait4IBE();
            SetPortVal(KBC_KEY_DATA, (IntPtr)MouseFun, 1);         //'发送鼠标动作命令
            KBCWait4IBE();
            SetPortVal(KBC_KEY_CMD, (IntPtr)211, 1);             // '发送鼠标写入命令
            KBCWait4IBE();
            SetPortVal(KBC_KEY_DATA, (IntPtr)MyMouseX, 1);         //  '发送鼠标动作命令
            KBCWait4IBE();
            SetPortVal(KBC_KEY_CMD, (IntPtr)211, 1);             // '发送鼠标写入命令
            KBCWait4IBE();
            SetPortVal(KBC_KEY_DATA, (IntPtr)MyMouseY, 1);         //  '发送鼠标动作命令
            KBCWait4IBE();
            SetPortVal(KBC_KEY_CMD, (IntPtr)211, 1);             // '发送鼠标写入命令
            KBCWait4IBE();
            SetPortVal(KBC_KEY_DATA, (IntPtr)MyMouseZ, 1);         //  '发送鼠标动作命令
        }
    }  
